...
class MQConnector {
        ...
        return new Promise((resolve, reject) =>{
            const performSetup = () => {
                ...
                .then(function (conn) {
                    ...
                    .then(function (ch) {
                        return ch.assertQueue(response_queue, { durable: false }).then(function (ok) {
                            return ch.consume(response_queue, function (msg) {
                                if (msg !== null) {
                                    // console.log(msg.content.toString);
                                    rabbitEmmiter.emit(msg.properties.correlationId, msg.content)
                                    ch.ack(msg);
                                    resolve();
                                }
                            });
    ...
    sendTask(data, uniqueID = '123') {
        ...
        return this.Connection.then(function (conn) {
            ...
            return ch.assertQueue(task_queue, { durable: false }).then(function (ok) {
                return ch.sendToQueue(task_queue, Buffer.from(stringifiedData), {correlationId: uniqueID });
            });
        }).catch(console.warn);
